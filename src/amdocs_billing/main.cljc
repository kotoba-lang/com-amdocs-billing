(ns amdocs_billing.main
  "Kotodama WASM entrypoint for the Amdocs_billing clean-room actor (L4) — Clojure port.
  ...
  facts (`amdocs_billing.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "amdocs_billing")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"amdocs_billing.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "amdocs_billing-compat" :tier tier :entities entities} 200])
...
(defn start! [] :amdocs_billing-compat/ready)
